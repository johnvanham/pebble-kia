...
// Touch gestures: pull down to force-refresh, swipe right to go back.
// Only emery's headers declare the real recognizer API — the other
// platforms stub the functions out as no-op macros, so this whole
// block compiles only where it can work.
...
static void window_load(Window *window) {
  ...
  window_attach_recognizer(
      window,
      swipe_recognizer_create(swipe_event, NULL, SwipeDirection_Right));
#endif
